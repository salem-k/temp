<?php
/**
 * @var \App\View\AppView $this
 * @var iterable<\App\Model\Entity\Commandegros> $commandegross
 */
?>
<div class="commandegross index content">
    <?= $this->Html->link(__('New Commandegros'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Commandegross') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('Date_debut') ?></th>
                    <th><?= $this->Paginator->sort('Date_din') ?></th>
                    <th><?= $this->Paginator->sort('repsommsects_id') ?></th>
                    <th><?= $this->Paginator->sort('repgros_id') ?></th>
                    <th><?= $this->Paginator->sort('repsectoriel_id') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($commandegross as $commandegros): ?>
                <tr>
                    <td><?= $this->Number->format($commandegros->id) ?></td>
                    <td><?= h($commandegros->Date_debut) ?></td>
                    <td><?= h($commandegros->Date_din) ?></td>
                    <td><?= h($commandegros->repsommsects_id) ?></td>
                    <td><?= $commandegros->has('repgro') ? $this->Html->link($commandegros->repgro->Responsable, ['controller' => 'Repgros', 'action' => 'view', $commandegros->repgro->id]) : '' ?></td>
                    <td><?= $commandegros->has('repsectoriel') ? $this->Html->link($commandegros->repsectoriel->Responsable, ['controller' => 'Repsectoriels', 'action' => 'view', $commandegros->repsectoriel->id]) : '' ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $commandegros->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $commandegros->id]) ?>
                        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $commandegros->id], ['confirm' => __('Are you sure you want to delete # {0}?', $commandegros->id)]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>
